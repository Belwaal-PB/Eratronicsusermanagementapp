...
class EratronicsApp {
    ...
    async refreshAdminData() {
        if (this.currentUser && this.currentUser.user_type === 'admin') {
            await this.loadData();
            // Refresh the admin dashboard display if it's currently shown
            this.showDashboard();
        }
    }
    ...
    async submitImageSelection() {
        ...
        if (confirm(`Are you sure you want to select ${selectedImages.length} image(s)?`)) {
            try {
                ...
                // If admin user, refresh the admin dashboard to show updated statistics
                if (this.currentUser && this.currentUser.user_type === 'admin') {
                    this.showDashboard();
                }
            } catch (error) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.success) {
                    ...
                    // Refresh the entire admin dashboard to update statistics
                    this.showDashboard();
